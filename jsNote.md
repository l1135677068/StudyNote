...
js代码->词法分析->预编译(得到执行上下文(变量环境存储变量提升的变量声明和函数声明定义, 词法环境包括let和const变量)和可执行代码)->执行(可执行代码)
...
js是单线程的，代码执行的时候分为执行栈和事件队列；非异步函数放入执行栈，异步函数放入消息队列中，当执行栈为空以后才去执行消息队列中的事件。为了区分优先级不同的事件，又引入了宏任务和微任务，所以执行顺序是执行栈中的任务(如果没有则从宏任务队列中取)=>当前宏任务中的微任务（promise.then)=>渲染线程=>下一个宏任务 （settimeout==）



#### this指向

1 如果函数作为对象的方法调用的时候，this指向调用的对象

2 内部函数的this不会指向外部函数

3 函数正常调用的时候，严格模式下this为undefined, 非严格模式为window
